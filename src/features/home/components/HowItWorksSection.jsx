...
const HowItWorksSection = () => {
  ...
  const data = [
    {title: "Teks Ilusif", description: "Mengubah warna teks menjadi putih seolah-olah spasi"},
    {title: "Gambar Paragraf", description: "Mengubah bagian paragraf menjadi gambar"},
    {title: "Teks Tersembunyi", description: "Menyembunyikan teks spam dibalik gambar"},
    {title: "Font Palsu", description: "Menggunakan font palsu untuk mengubah teks"},
  ]
        ...
        <div className="container mx-auto py-20 px-10">
          ...
          <div className="flex flex-wrap">
            <div className="basis-5/12 pt-10 hidden lg:block">
              {data.map((item, index) => (
                <MethodCard title={item.title} description={item.description}
                            active={activeMethod === index} onClick={() => dispatch(setActiveMethod(index))} />
              ))}
            </div>
            <div className="basis-full block lg:hidden">
              <div className="flex flex-wrap items-center justify-center mb-2">
                {data.map((item, index) => (
                  <SimpleMethodCard title={item.title} active={activeMethod === index}
                                    onClick={() => dispatch(setActiveMethod(index))} />
                ))}
              </div>
              <p className="text-center mb-4">{data[activeMethod].description}</p>
            </div>
